...
public class Normal_Login {
	...
	public void Login_Test() throws InterruptedException {
		...
		if (tranggle_Home.equals("Tranggle")) {
			System.out.println("Login_Test() - Tranggle Homepage Move Success : " + tranggle_Home);

		} else {
			System.out.println("Login_Test() - Tranggle Homepage Move Failure : " + tranggle_Home);
		...
		if (rnb_Login_Text.equals("로그인")) {
			System.out.println("Login_Test() - Tranggle RNB Open Success : " + rnb_Login_Text);
		} else {
			System.out.println("Login_Test() - Tranggle RNB Open Failure : " + rnb_Login_Text);
		...
		if (id_Search_Text.equals("아이디 찾기")) {
			System.out.println("Login_Test() - Tranggle Loginpage Move Success : " + id_Search_Text);
		} else {
			System.out.println("Login_Test() - Tranggle Loginpage Move Failure : " + id_Search_Text);
		...
		if (Logout_Text_Btn.getText().equals("로그아웃")) {
			System.out.println("Login_Test() - Tranggle Normal Login Success : " + Logout_Text_Btn.getText());
		} else {
			System.out.println("Login_Test() - Tranggle Normal Login Failure : " + Logout_Text_Btn.getText());
	...
	public void Logout() throws Exception {
		...
		// 트랭글 홈페이지 이동 확인 체크
		if (tranggle_Home.equals("Tranggle")) {
			System.out.println("Normal_Login : Logout() - Tranggle Homepage Move Success : " + tranggle_Home);

		} else {
			System.out.println("Normal_Login : Logout() - Tranggle Homepage Move Failure : " + tranggle_Home);
		...
		if (Logout_Text_Btn.getText().equals("로그아웃")) {
			System.out
					.println("Normal_Login : Logout() - Tranggle Normal Login Success : " + Logout_Text_Btn.getText());
		} else {
			System.out
					.println("Normal_Login : Logout() - Tranggle Normal Login Failure : " + Logout_Text_Btn.getText());

			throw new InterruptedException("RNB 화면 펼침 실패 or RNB 화면 > [로그아웃] 버튼 앨리먼튼 찾기 실패했습니다.");
		}

		// 로그아웃 처리를 위해 [로긍아웃] 버튼을 클릭
		Logout_Text_Btn.click();

		// 정상 로그아웃 처리를 위한 7초 대기
	...
	public void EndDriver() throws Exception {
		...
		/*
		 * 아래의 작업을 이유 : 로그인을 하고 나면 로그인 상태를 유지 하기 떄문에, 다른 테스트 스크립트를 돌릴 경우, 로그인페이지를 보거나 하는
		 * 케이스 경우 확인을 할 수 없어,
		 * 
		 * 다른 테스트 스크립트에 영향을 받기 때문에, 테스트 스크립트 종료 전 미리 로그아웃 처리 작업을 진행한다.
		 */
